...
/* Grille de colonnes : une colonne par mois, 3-4 colonnes selon la largeur d'écran */
.mois-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 14px; align-items: start; }
.mois-colonne { display: flex; flex-direction: column; gap: 8px; }
.mois-titre { text-align: center; font-size: 11px; font-weight: bold; color: #1a4a7a; background: #e3ebf6; border-radius: 6px; padding: 5px; letter-spacing: 1px; text-transform: uppercase; }

.ord-card { background: white; border-radius: 8px; padding: 8px 10px; box-shadow: 0 1px 4px rgba(0,0,0,0.1); border-left: 4px solid #1a4a7a; transition: box-shadow 0.2s; }
.ord-card:hover { box-shadow: 0 3px 12px rgba(0,0,0,0.15); }
.ord-card.recent { border-left-color: #27ae60; }

.ord-header { display: flex; align-items: center; gap: 6px; margin-bottom: 6px; flex-wrap: wrap; }
.ord-num { font-size: 10px; color: #888; }
.ord-date { font-size: 12px; font-weight: bold; color: #1a4a7a; }
.ord-acte { background: #1a4a7a; color: white; padding: 1px 7px; border-radius: 8px; font-size: 10px; font-weight: bold; }
.ord-rdv { background: #e8d5f5; color: #8e44ad; padding: 1px 7px; border-radius: 8px; font-size: 10px; font-weight: bold; }
.ord-heure { background: #f0f4f8; color: #555; padding: 1px 6px; border-radius: 8px; font-size: 10px; }
...
<div class="container">
    ...
    // Étape 1 : regrouper toutes les ordonnances par mois (un tableau de blocs par mois)
    $parMois = [];
    foreach ($ordonnances as $idx => $ord) {
        $tsOrd     = strtotime($ord['date_ordon'] ?? '');
        $moisAnnee = ($tsOrd && $tsOrd > 86400) ? ($moisFr[(int)date('n', $tsOrd)] . ' ' . date('Y', $tsOrd)) : 'Date inconnue';
        $parMois[$moisAnnee][] = ['ord' => $ord, 'idx' => $idx, 'ts' => $tsOrd];
    }
    ?>
    <div class="mois-grid">
        <?php foreach ($parMois as $moisAnnee => $blocs): ?>
        <div class="mois-colonne">
            <div class="mois-titre"><?= htmlspecialchars($moisAnnee) ?></div>
            ...
            <div class="ord-card <?= $isRecent ? 'recent' : '' ?>">
                <div class="ord-header">
                    ...
                    <?php endif; ?>
                    <?php if ($dateRdv !== '—'): ?>
                    <span class="ord-rdv">📆 <?= $dateRdv ?></span>
                    <?php endif; ?>
                    <?php if ($heure): ?>
                ...
                <div class="meds-list">
                    ...
                    <div class="med-ligne">
                        ...
                        <span class="med-duree"><?= htmlspecialchars(str_ireplace('Traitement', 'Tt', $m['DUREE'] ?? '')) ?></span>
                    </div>
                    <?php endforeach; ?>
                </div>
                <?php else: ?>
